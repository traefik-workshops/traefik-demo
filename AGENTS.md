...
Don't guess `traefik.io` / `hub.traefik.io` fields. The authoritative, versioned, field-level reference for everything Traefik + Hub (CRDs, middlewares, providers, static config) lives in the **private `traefik/reference`** repo and is consumed on demand by [`scripts/reference.sh`](./scripts/reference.sh) — nothing is vendored here (it's private; this repo is public).

- **Look up a concept**: `make reference PAGE=hub/crd/apiplan` (also `oss/middlewares/jwt`, `hub/middlewares/oidc`, or `INDEX` for the discovery index). Each page lists exact fields, types, enums, and defaults.
- **Validate manifests**: `make helm-template` (part of `make check`) renders every chart and validates the emitted Traefik/Hub custom resources against the real JSON Schemas with kubeconform. `make reference-schemas` just refreshes the cache (`.reference/`, gitignored).
- **Auth & degradation**: needs `gh auth login` with read access to `traefik/reference`. Unauthenticated, every entry point degrades — it prints how to authenticate, then continues *without* the reference rather than blocking. Pin a snapshot with `REFERENCE_REF=<sha>`.

## Where to look next (dev / maintenance)
...
- **Traefik/Hub CRD fields**: `make reference PAGE=<id>` → see [Traefik + Hub config & CRD reference](#traefik--hub-config--crd-reference) above.
- Human-readable module index: [`CATALOG.md`](./CATALOG.md) — scannable by section, with common PoC stacks. Generated by `make catalog-markdown`.
...
- Dev skills: [`new-module`](./.claude/skills/new-module/README.md), [`new-chart`](./.claude/skills/new-chart/README.md), [`bump`](./.claude/skills/bump/README.md)
...
- SA demo workflows → see [`sa-assistant`](./.claude/skills/sa-assistant/SKILL.md) skill.
