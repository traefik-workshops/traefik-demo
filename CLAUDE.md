...
**A change is breaking if any downstream consumer would need to edit their `module {}` block or `values.yaml` to keep working.** When unsure, treat as breaking. Helm-specific breaking changes are listed in [`helm/CLAUDE.md`](./helm/CLAUDE.md#when-changing-an-existing-chart).
